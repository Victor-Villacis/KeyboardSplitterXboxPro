...
describe("the macro editor's duration controls", () => {
  ...
  // Victor: "the time selection is like selecting the step, so maybe we can
  // abstract the selection?" The mode was the bug: one duration field under
...
describe("diagonals are a lens over the stored pair", () => {
  ...
  test("ticking ↓ then → says the two are now a diagonal", async () => {
    // THE FOLD MOMENT — the one report nobody got, and the exact sentence
    // Victor lost an evening to.
    //
...
// Victor: "when I remove all the steps by pressing x there is no way to add
// them back." Every add affordance lived on a row (＋↑ / ＋↓), so the last ✕
